...
生成に失敗しても録画の結末は変わりません。作る側は台帳の `thumbnail_state` が `Pending` の行を
自分で拾いに行くので、録画を完了させる経路からサムネイルを呼ぶ箇所はありません。失敗した理由は
`thumbnail_fault` に分類つきで残します — 実行ファイルが無い / 録画ファイルに届かない / 終了コードが
0 でない / 時間切れ / 成功と言って何も置かなかった、の5つを区別します。
