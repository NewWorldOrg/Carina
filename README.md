...
| `CARINA_DRI_VIDEO_GID` | `card0` を開ける補助グループ。ホストでの実測値 |
| `CARINA_DRI_RENDER_GID` | `renderD128` を開ける補助グループ。ホストでの実測値 |
...
そのため `app` の役割は、コンテナの中に実際に在るデバイスノードの所有グループを読み、降格後のグループにそれだけを加えます。
番号の出所はノードそのものなので、名前でも環境変数でもホストとずれません。
ノードが無ければ何も加わらず、所有グループが `0` のノードは理由を述べて渡しません。
