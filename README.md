...
**誰も頼まないジョブは、退役していない保存先がちょうど 1 つのときだけ作られる。**
その保存先が名乗る既定のプロファイルで焼く。
保存先が 1 つも無いとき、2 つ以上あるとき、既定のプロファイルが退役しているときは、**何も作らず、その理由をログに残すだけで止まる**。
どれで焼くかは人が決めることなので、ここでは選ばない。
録画ごと・予約ごとにプロファイルを選ぶ口は持たない。
