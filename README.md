...
録画をあとから軽く観られる形に焼き直します。成果物は **app が書き込みで持つ別のルート**に置き、
録画が書かれた出力ルートには置きません。録画の出力ルートは driver だけが書く場所で、app には
読み取り専用で渡っているためです。どのルートを持つかは `Encodings:OutputRoots` で
`名前=/絶対パス` を `;` で並べて教えます。1つも書かなければ何もエンコードされず、その旨を
起動時に一度だけ書き残します。

`GET /api/storage` が返す出力ルートの集合は、driver が宣言するものの後ろに app が持つこれらの
ルートを並べたものです。空き容量と総容量はディスクから、書けるかどうかは実際に rename を試して
測ります。driver と同じ名前を app 側にも書くと、その名前は driver のものとして扱い、app 側の
ルートは集合から外して警告します。保存先が名指しできるのはこの集合のうち app が持つルートだけで、
録画の出力ルートを名指しした保存先は保存時に拒否されます。

作業ファイルは成果物と同じルートの下に書き、完成時に rename します。`Encodings:WorkedIn` に
作業ディレクトリを書くと全ルート共通でそこに書きますが、いずれかのルートと別のマウントにあれば
プロセスは起動しません(別マウントをまたぐ rename は copy に化け、途中で止まっても成功に見えるため)。
...
| `Encodings:MostCores` | `2` | 1本のジョブが使うコア数の上限(ライブ視聴に譲る) |
...
コンテナで動かすなら、このルートは app だけに読み書きで渡します(`compose.yml` の `encodes`
ボリューム)。`app` ロールは非特権ユーザーで動くので、そのユーザーが書けるディレクトリであることが
条件です。

### 信号品質の採取

driver がセッションを保持しているあいだだけ、その frontend が返す lock 状態・CNR・階層別の
post-Viterbi 誤りビット数を貯めます。**品質を測るためにチューナーを掴むことはありません** —
`GET /tuners` を読むだけで、セッションを開く口もストリームを開く口も呼びません。誰かの録画が
計測のために止まることが無いようにするためで、規約テストで固定しています。

lock していない frontend の CNR は値として採りません(無信号でも負の値が返るため)。lock と
統計は取得時刻が別なので、それぞれの時刻を添えて残します。階層別の誤りビット数は階層のまま
残し、1つに畳みません。カウンタは選局のたびに巻き戻るので、サンプルはセッションを名乗り、
その**境界をまたぐ差分は計算しません**。

読みを受け取れなかったときは捨てず、**「取得できず」として、なぜ取れなかったかの分類つきで**
残します。黙って消すと計測が止まっていることが見えなくなるためです。

貯めた生値は分と時間の2つの窓に集約し、**集約されていない生値は消しません**。保持期間は
生値・分の窓・時間の窓のそれぞれに設定を持ちます。`forever` と書いた窓は消えません。
