...
| `Transcoding:CaptionDelay` | `00:00:00` | 字幕を映像に対して遅らせる(負なら早める)量。`-00:00:10` から `00:00:10` の間で、それより外や時間でない値は起動を停止する |

### 字幕

放送の字幕(ARIB STD-B24)は、ブラウザに渡す前にサーバで絵にします。ライブ1本につき、映像を変換する
ffmpeg とは別に字幕だけを描く ffmpeg をもう1本起こし、字幕が変わるたびにその絵をパレット PNG(color type 3)
にして、WebSocket の字幕チャネルで配ります。描く側は字幕以外のストリームを復号しないので、変換本数の予算
(`Transcoding:AtOnce`)には数えません。字幕を描くほうが起動しなくても、視聴は字幕なしで続きます。
...
字幕の時刻は放送の 90kHz の時計そのままで、映像のフラグメントと同じ時計です。字幕はエンコードを通らないぶん
映像より先に届くので、時計で合わせる受け側には補正は要りませんが、届いた順に出す受け側や環境に合わせて
`Transcoding:CaptionDelay` でずらせます。途中から観る人には、いま出ている字幕を1枚だけ取り置いて、
ヘッダの直後に送ります。字幕が消えたときは空の字幕フレームが届き、取り置きも消えます。
