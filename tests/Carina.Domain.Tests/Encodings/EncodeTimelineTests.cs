...
public sealed class EncodeTimelineTests
{
    ...
    [Fact(DisplayName = "the artefact's zero on the source's clock is the source's start plus the head skipped — 30499.474078 + 0.5072 = 30499.981278, the first picture that could be decoded")]
    ...
    [Fact(DisplayName = "an artefact within a second of what the source had left agrees, one further off does not, and neither is anything but a reading")]
    ...
    [Fact(DisplayName = "a source whose length was not measured has nothing for the artefact to agree with, and says so rather than agreeing")]
    ...
    [Fact(DisplayName = "a head is skipped by between nothing and five seconds; the broadcast clock handed in as a skip is refused, because that is the seventeen hours")]
...
{
    [Fact(DisplayName = "the head skip is the first decodable picture less the container's start, on the source's own clock")]
    ...
    [Fact(DisplayName = "a head that could not be read is a fault with a note, never a skip of nothing")]
