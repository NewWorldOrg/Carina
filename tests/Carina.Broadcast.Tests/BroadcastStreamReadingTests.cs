...
public sealed class BroadcastStreamReadingTests
{
    ...
    {
        IReadOnlyList<byte[]> network = new TransportStreamWriter(NetworkInformationTable.Pid)
            .Sections(NitSection(version: 1))
            .Packets;
        IReadOnlyList<byte[]> services = new TransportStreamWriter(ServiceDescriptionTable.Pid)
        ...
        SectionRead[] read = Interleave(network, services).SelectMany(packet => reader.Push(packet)).ToArray();

        Assert.All(read, outcome => Assert.IsType<SectionRead.Assembled>(outcome));

        SectionRead.Assembled[] sections = read.Cast<SectionRead.Assembled>().ToArray();
        NetworkInformationTable nit = Assert.IsType<TableRead<NetworkInformationTable>.Parsed>(
            NetworkInformationTable.Read(sections.Single(section => section.Pid == NetworkInformationTable.Pid)
                .Section)).Table;
        ServiceDescriptionTable sdt = Assert.IsType<TableRead<ServiceDescriptionTable>.Parsed>(
    ...
    {
        byte[] section = SdtSection(version: 1, serviceCount: 40);
        IReadOnlyList<byte[]> packets = new TransportStreamWriter(ServiceDescriptionTable.Pid).Sections(section).Packets;

        Assert.True(packets.Count > 1);

        var reader = new SectionReader(ServiceDescriptionTable.Pid);
        SectionRead[] read = packets.SelectMany(packet => reader.Push(packet)).ToArray();
        ServiceDescriptionTable table = Assert.IsType<TableRead<ServiceDescriptionTable>.Parsed>(
    ...
    public void ASectionWhoseChecksumFailsLeavesTheTableIncompleteUntilItIsSentAgain()
    {
        ...
        IReadOnlyList<SectionRead> broken = stream.Carry(
        ...
        Assert.True(set.TryComplete(out IReadOnlyList<Section>? sections));
    ...
    private sealed class ServiceStream
    {
        ...
        public IReadOnlyList<SectionRead> Carry(byte[] section)
        {
            ...
            foreach (SectionRead.Assembled assembled in read.OfType<SectionRead.Assembled>())
            {
    ...
    {
        for (int at = 0; at < Math.Max(first.Count, second.Count); at++)
        {
            if (at < first.Count)
            {
