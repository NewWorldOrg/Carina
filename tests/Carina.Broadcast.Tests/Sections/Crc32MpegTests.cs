...
public sealed class Crc32MpegTests
{
    ...
    {
        byte[] data = new byte[] { 0x40, 0xF0, 0x11, 0x00, 0x01, 0xC1, 0x00, 0x00 };
        uint checksum = Crc32Mpeg.Compute(data);

        byte[] carried = new byte[data.Length + 4];
    ...
    {
        byte[] data = new byte[] { 0x42, 0xF0, 0x20, 0x00, 0x01, 0xC1, 0x00, 0x00 };
        uint checksum = Crc32Mpeg.Compute(data);
